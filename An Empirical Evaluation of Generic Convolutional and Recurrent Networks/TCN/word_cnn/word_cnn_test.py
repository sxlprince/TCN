...
print ("参数：", args)
#print(args)
corpus = data_generator(args)             #包括train  valid  test
print ("训练集所有句子单词个数：",corpus.train.size(0))  #929589
print ("验证集所有句子单词个数：",corpus.valid.size(0))  #73760
print ("测试集所有句子单词个数：",corpus.test.size(0))   #82430
eval_batch_size = 10
train_data = batchify(corpus.train, args.batch_size, args)   # 16 * 58099 tensor
print ("训练集shape：", train_data.size())
val_data = batchify(corpus.valid, eval_batch_size, args)     #10 * 7376  tensor
print ("验证集shape：", val_data.size())
test_data = batchify(corpus.test, eval_batch_size, args)     #10 * 8243  tensor
print ("测试集shape：", test_data.size())

n_words = len(corpus.dictionary)      #不重复单词个数  我猜包含了三个集合所有数据集？？
print ("词汇表大小：", n_words)


num_chans = [args.nhid] * (args.levels - 1) + [args.emsize]  #[600,600,600,600]
k_size = args.ksize                   #3
dropout = args.dropout                #0.45
emb_dropout = args.emb_dropout        #0.25
tied = args.tied
