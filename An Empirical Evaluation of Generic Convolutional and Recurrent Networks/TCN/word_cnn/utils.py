...
def data_generator(args):
    if os.path.exists(args.data + "/corpus") and not args.corpus:
        ...
        corpus = Corpus(args.data)               #corpus分为train valid test
...
class Corpus(object):                #object = ./data/penn
    ...
    def tokenize(self, path):
        ...
        with open(path, 'r') as f:
            ...
            for line in f:
                ...
                tokens += len(words)
                for word in words:
                    self.dictionary.add_word(word)     #返回词汇表字典
        ...
        return ids                 #返回标记的数据集   [num, ]


def batchify(data, batch_size, args):     #batchify(corpus.train, args.batch_size（16）, args)
    """The output should have size [L x batch_size], where L could be a long sequence length"""
    # Work out how cleanly we can divide the dataset into batch_size parts (i.e. continuous seqs).
    nbatch = data.size(0) // batch_size    #训练集所有句子单词个数总和 / batch_size
    # Trim off any extra elements that wouldn't cleanly fit (remainders).
    data = data.narrow(0, 0, nbatch * batch_size)
    # Evenly divide the data across the batch_size batches.
    data = data.view(batch_size, -1)                   #16 * 58099
    if args.cuda:
        data = data.cuda()
    return data

#get_batch(train_data, i, args)  #16 * 58099     0,40,80...   args
def get_batch(source, i, args, seq_len=None, evaluation=False):
    seq_len = min(seq_len if seq_len else args.seq_len, source.size(1) - 1 - i)  #最后一个数据长度58
